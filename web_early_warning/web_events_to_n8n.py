...
from web_early_warning.feature_pipeline import FEATURE_PROFILES
...
def _extract_payload(obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Prefer the richer profile keys when a flat event is provided.
    rich_keys = set(FEATURE_PROFILES.get("web_monitor_60_v1", FEATURE_PROFILES["web_monitor_rich_v2"]))
    base_keys = {
        "Flow Duration",
        "Total Fwd Packet",
        "Total Bwd packets",
...
def main() -> int:
    ...
    ap.add_argument(
        "--events-path",
        default=os.getenv("WEB_EVENTS_PATH", "/var/log/web_threat/events.jsonl"),
