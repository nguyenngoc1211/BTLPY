...
COLLECTOR_SERVICE="${COLLECTOR_SERVICE:-web_early_warning_web_collector.service}"
...
section "Scoring API"
if curl -fsS --max-time 3 "$SCORE_URL" >/tmp/web_score_health.json 2>/dev/null; then
  pass "Reachable: $SCORE_URL"
  printf '  response=%s\n' "$(tr -d '\n' < /tmp/web_score_health.json | head -c 300)"
else
