...
FEATURE_PROFILES: Dict[str, List[str]] = {
    ...
    # Balanced profile for production: richer than 18 features but still robust
    # when realtime collector only provides a subset (missing columns are zero-filled).
    "web_monitor_60_v1": [
        "FlowDuration",
        "TotFwdPkts",
        "TotBwdPkts",
...
def normalize_label_binary_attack(x: object) -> str | None:
    s = str(x).strip().upper()
    if not s:
        return None
    if s == "BENIGN":
        return "Benign"
    # Any non-benign label is treated as attack in binary web monitoring mode.
    return "WebAttack"
