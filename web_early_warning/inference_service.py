...
from web_early_warning.feature_pipeline import transform_features
...
def _default_bundle_path() -> str:
    ...
    if env:
        return env
    preferred = "web_early_warning/model_out_flowfeatures/lgbm_combined_flow_web_60_webattack.joblib"
    return preferred


def _default_alerts_db_path() -> str:
    return os.getenv("ALERTS_DB_PATH", "/var/log/web_threat/alerts.db")
...
def _canonical_stage_name(x: str) -> str:
    s = str(x).strip()
    up = s.upper()
    if not s:
        return s
    if up == "BENIGN":
        return "Benign"
    if up in {"ATTACK", "WEBATTACK", "WEB_ATTACK"}:
        return "WebAttack"
    # Backward compatibility: map any non-benign stage labels to WebAttack in binary mode.
    return "WebAttack"


def _is_binary_web_attack_setup(labels: List[str]) -> bool:
    s = {_canonical_stage_name(x) for x in labels}
    return "Benign" in s and "WebAttack" in s and len(s) <= 2


def _policy_defaults(labels: List[str]) -> Dict[str, Any]:
    if _is_binary_web_attack_setup(labels):
        # Binary setup (Benign/WebAttack): aggressively alert if model predicts attack.
        return {
            "alert_labels_csv": "WebAttack",
            "min_conf_high_impact": 0.50,
            "min_conf_non_benign": 0.50,
            "policy_mode": "binary_web_attack",
        }
    return {
...
app = FastAPI(title="Web Threat Early Warning - Stage Scoring API")
...
def decide(stage: str, conf: float, labels: List[str]) -> tuple[str, str]:
    ...
    alert_stages = {
        _canonical_stage_name(x)
        for x in _env_csv_list(
            "ALERT_LABELS",
            os.getenv("ALERT_STAGES", default_alert_csv),
        )
    }
    stage = _canonical_stage_name(stage)
...
def score(req: ScoreRequest) -> ScoreResponse:
    ...
    stage = _canonical_stage_name(bundle.label_names.get(stage_id, str(stage_id)))
    conf = float(proba[best_idx])

    proba_map: Dict[str, float] = {}
    for idx, class_id in enumerate(classes):
        key = _canonical_stage_name(bundle.label_names.get(int(class_id), str(class_id)))
        if idx < len(proba):
            proba_map[key] = max(float(proba[idx]), proba_map.get(key, 0.0))
